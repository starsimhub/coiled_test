...
def build_sim(sim, calib_pars, **kwargs):
    ...
    ms = ss.MultiSim(sim,
                     iterpars=dict(rand_seed=np.random.randint(0, 1e6, reps)),
...
def extract_prevalence(sim):
    df = pd.DataFrame({
        ...
        # Instead of prevalence, let's compute it from infected and n_alive
...
def test_coiled(do_plot=True):
    sc.heading(
        'Testing a single parameter (beta) with a normally distributed likelihood, one data point')

    # Define the calibration parameters
    calib_pars = dict(
        beta=dict(low=0.01, high=0.30, guess=0.15, suggest_type='suggest_float',
                  log=True),
    )
